...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     */
    def never[T]: Future[T] = Promise[T].failure(new TimeoutException).future
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      ...
      case x::xs => x.flatMap(t => all(xs).flatMap(ts => Future{t::ts}))
    }
     ...
     */
    def delay(t: Duration): Future[Unit] = Future { blocking {Thread.sleep(t.toMillis); Unit} }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val r = CancellationTokenSource()
      f(r.cancellationToken)
      r
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = Await.result(f, Duration(0, MILLISECONDS))
    ...
    def continue[S](cont: Try[T] => S): Future[S] = Future{
      cont (Promise[T].completeWith(f).future.value match {
        case Some(x: Try[T]) => x
      })
    }
