...
class Step3_ReplicatorSpec extends TestKit(ActorSystem("Step3ReplicatorSpec"))
  ...
  test("case3: Replicator should have no outstanding acks after a successful SnapshotAck") {
    val secondary = TestProbe()
    val replicator = system.actorOf(Replicator.props(secondary.ref), "case3-replicator")
    val ackChecker = TestProbe()

    ackChecker.send(replicator, GetNumAcks)
    ackChecker.expectMsg(NumAcks(0))
  ...
  test("case4: Replicator should have no outstanding acks after a timeout") {
    val secondary = TestProbe()
    val replicator = system.actorOf(Replicator.props(secondary.ref), "case4-replicator")
    val ackChecker = TestProbe()
    ...
    ackChecker.send(replicator, GetNumAcks)
    ackChecker.expectMsg(NumAcks(1))
