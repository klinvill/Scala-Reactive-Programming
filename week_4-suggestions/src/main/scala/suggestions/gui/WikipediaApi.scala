...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(s => Try(s)).onErrorReturn(e => Failure(e))
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      obs.take(Duration(totalSec, SECONDS))
    }
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.flatMap(requestMethod).map(x => Try(x))
