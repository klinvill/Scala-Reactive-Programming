...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = Observable.create (obs => {
      button.clicks.subscribe(
        btn => suggestionList.selection.items.headOption match {
          case Some(b) => obs.onNext(b)
          case None => status.text = "No selection detected"
        }
      )
    })
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(x: String) => editorpane.text = x
        case Failure(e) => status.text = "Failed to subscribe to page: " + e.getMessage()
      }
    }
